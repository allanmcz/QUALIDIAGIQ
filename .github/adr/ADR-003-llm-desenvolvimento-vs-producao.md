...
- **`QDI_LLM_BACKEND`** + **`ANTHROPIC_API_KEY`** em `Settings` materializam a fábrica em `build_llm_adapter_from_settings()` (**ADR-021**, `llm_adapter_factory.py`), invocada por `get_llm_service()` (`deps_infra_services.py`): `anthropic` com chave ⇒ `AnthropicLlmAdapter`; sem chave ⇒ fallback **LangGraph/Ollama** com log estruturado (`llm_backend_anthropic_sem_api_key`).
