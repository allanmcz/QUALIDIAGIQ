...
**P6 / RLS (clareza):** o **smoke automatizado** de dois tenants está em `tests/integration/test_mvp_gate_postgres.py` e corre contra **PostgreSQL com as migrações do repo** — em desenvolvimento típico isso é o **Docker Compose** (`localhost:60322`, ver `make dev` / `QDI_POSTGRES_TEST_URL`). Ou seja: valida **as mesmas políticas SQL** que se aplicam no caminho Supabase (JWT + `authenticated`), mas **não** é, por si só, login no **projeto Supabase hospedado** (dashboard/cloud). A secção 1 separa linha **`[x]` automação Docker/CI** de **`[ ]` evidência opcional no projeto cloud**.

**Distinção importante:** o quadro acima é **decisão de produto/arquitetura**. Continua **pendente de execução/evidência operacional** o que está na **secção 1** (ex.: sign-off contábil P5, espelho WeasyPrint, evidência RLS no **Supabase hospedado** se quiseres parity documentada com prod, **tag/release/changelog**). Na secção 1 já constam como **feitas / confirmadas**: migrações **0015** e **0016**, **idioma do relatório PDF** (**pt-BR** + **en**), e **RLS dois tenants no Postgres de desenvolvimento/CI** (Docker Compose ou URL equivalente).
...
| **1** | P6 — RLS smoke 2 tenants **automatizado** (Postgres **Docker Compose** `:60322` / CI; mesmas migrações) — **estado:** coberto por teste | Allan (aceita critério de paridade SQL com Supabase PG) | Eng / CI — `make mvp-gate`, `pytest … test_mvp_gate_postgres.py` |
...
| [x] | **P6 — RLS** smoke **dois tenants** em **Postgres com migrações QDI** (Docker Compose **`60322`** ou CI / `QDI_POSTGRES_TEST_URL`) | `tests/integration/test_mvp_gate_postgres.py`, `make mvp-gate`, `docs/operacao/SMOKE_MVP_FECHADO.md` | 2026-05-02 | Políticas `0003_rls_policies` + role `authenticated` + JWT simulado; **é o fluxo “local no Docker”** que referiste — equivalente semântico ao RLS Supabase self-hosted PG. |
| [ ] | **P6 — RLS** evidência **no projeto Supabase hospedado** (cloud), se exigires parity explícita com prod | `docs/operacao/RUNBOOK_DEPLOY_ROLLBACK.md`, `_DEVELOPER/analises/GAP_ANALYSIS_RLS_P6_2026-05-02.md` | | **Opcional** para MVP técnico fechado no repo; preenche quando houver captura/SQL ou nota Ops no projeto real. Podes **dispensar** com decisão explícita (“aceito evidência Docker/CI”). |
...
| [ ] | **Tag / release** MVP + linha em `docs/CHANGELOG_MVP.md` | `_DEVELOPER/HANDOFF_PLANO_MVP_FECHADO.md` §8 | | **Ainda não definido** — sem tag Git, sem versão de release acordada nem entrada no changelog; Allan define momento e identificador (ex. `v1.0.0-mvp`). |
