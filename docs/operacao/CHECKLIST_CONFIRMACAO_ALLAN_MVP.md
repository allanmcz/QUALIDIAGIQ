...
| **1** | P6 — RLS evidência no **Supabase hospedado** (projeto cloud) | Allan (**aceita** evidência ou **dispensa** explicitamente) | Ops / credenciais — `RUNBOOK_DEPLOY_ROLLBACK.md`, `GAP_ANALYSIS_RLS_P6_2026-05-02.md` |
...
| [ ] | **P6 — RLS** evidência **no projeto Supabase hospedado** (cloud), se exigires parity explícita com prod | `docs/operacao/RUNBOOK_DEPLOY_ROLLBACK.md`, `docs/operacao/GAP_ANALYSIS_RLS_P6_2026-05-02.md` | | **Opcional** para MVP técnico fechado no repo; preenche quando houver captura/SQL ou nota Ops no projeto real. Podes **dispensar** com decisão explícita (“aceito evidência Docker/CI”). |
