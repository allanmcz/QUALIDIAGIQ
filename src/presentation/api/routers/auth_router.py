...
async def cadastro_consultor_b2b(body: CadastroConsultorB2BRequest) -> LoginResponse:
    ...
    try:
        ...
        if dsn_cad:
            if buscar_admin_por_email_postgres(email_norm, dsn_cad):
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail="Este e-mail já está cadastrado.",
                )
            try:
                novo_id = inserir_admin_postgres(
                    email=email_norm,
        ...
        logger.info("cadastro_consultor_b2b_ok", email=email_norm)
        return LoginResponse(
            access_token=access_token,
            token_type="bearer",
            nome=nome_limpo,
        ...
        logger.exception("cadastro_postgres_erro", erro=str(e))
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=(
        ...
        logger.exception("cadastro_jwt_emit_erro", erro=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Falha ao emitir o token de sessão após cadastro.",
        ) from e
    except Exception as e:
        logger.exception("cadastro_consultor_falhou", tipo=type(e).__name__, erro=str(e))
        msg = str(e).lower()
