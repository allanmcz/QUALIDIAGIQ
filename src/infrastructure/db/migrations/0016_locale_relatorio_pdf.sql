-- Idioma do relatório PDF (WeasyPrint) + extensão WORM para coluna imutável após finalizado.

ALTER TABLE diagnosticos ADD COLUMN IF NOT EXISTS locale_relatorio VARCHAR(16) NOT NULL DEFAULT 'pt-BR';

COMMENT ON COLUMN diagnosticos.locale_relatorio IS
    'Idioma do relatório PDF: pt-BR (padrão) ou en (labels EN; conteúdo dinâmico pode permanecer PT até tradução completa).';
