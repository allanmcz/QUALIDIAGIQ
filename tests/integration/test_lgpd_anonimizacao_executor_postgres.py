"""
Integração PostgreSQL — executor de anonimização LGPD (adapter síncrono + WORM 0029).

Requer instância com migrações aplicadas (`make dev` + `make migrate`) ou, no mínimo,
tabelas `diagnosticos`, `lgpd_titular_solicitacao` e ficheiro 0029 para criar
`lgpd_anonimizacao_log` + função WORM.
