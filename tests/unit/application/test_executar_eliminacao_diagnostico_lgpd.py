"""Testes do caso de uso de eliminação física de diagnóstico pós-deferimento LGPD."""
...
class _RecordingExecutor(LgpdEliminacaoExecutorPort):
...
def _sol_elim(
    *,
    ...
    solicitacao_id: UUID,
    diagnostico_id: UUID | None,
    tipo: TipoSolicitacaoTitular = TipoSolicitacaoTitular.ELIMINACAO,
...
class TestExecutarEliminacaoDiagnosticoLgpd:
    @pytest.mark.asyncio
    async def test_executor_chamado_quando_valido(self) -> None:
        ...
        diag_id = uuid4()
        actor = uuid4()
        row = _sol_elim(tenant_id=tenant, solicitacao_id=sol_id, diagnostico_id=diag_id)
        port = _FakePort(row)
        ex = _RecordingExecutor()
        uc = ExecutarEliminacaoDiagnosticoLgpd(port_solicitacoes=port, executor=ex)
        cmd = ComandoExecutarEliminacaoDiagnosticoLgpd(
            tenant_id=tenant,
            actor_user_id=actor,
    ...
    async def test_rejeita_tipo_errado(self) -> None:
        tenant = uuid4()
        row = replace(
            _sol_elim(tenant_id=tenant, solicitacao_id=uuid4(), diagnostico_id=uuid4()),
            tipo=TipoSolicitacaoTitular.ACESSO,
        )
        uc = ExecutarEliminacaoDiagnosticoLgpd(
            port_solicitacoes=_FakePort(row), executor=_RecordingExecutor()
        )
        with pytest.raises(ValueError, match="eliminação"):
            await uc.execute(
                ComandoExecutarEliminacaoDiagnosticoLgpd(
                    tenant_id=tenant,
                    actor_user_id=uuid4(),
    ...
    async def test_rejeita_diag_divergente(self) -> None:
        tenant = uuid4()
        row = _sol_elim(tenant_id=tenant, solicitacao_id=uuid4(), diagnostico_id=uuid4())
        uc = ExecutarEliminacaoDiagnosticoLgpd(
            port_solicitacoes=_FakePort(row), executor=_RecordingExecutor()
        )
    ...
    async def test_rejeita_sem_diagnostico_vinculado(self) -> None:
        tenant = uuid4()
        sol_id = uuid4()
        row = _sol_elim(tenant_id=tenant, solicitacao_id=sol_id, diagnostico_id=None)
        uc = ExecutarEliminacaoDiagnosticoLgpd(
            port_solicitacoes=_FakePort(row), executor=_RecordingExecutor()
        )
