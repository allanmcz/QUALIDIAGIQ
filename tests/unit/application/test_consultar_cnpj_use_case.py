...
@pytest.mark.asyncio
async def test_merge_sem_alteracao_nao_chama_atualizar_repo() -> None:
    """``hist`` vazio quando cadastro já coincide com ``payload_canonico``."""
    exp = datetime.now(UTC)
    tid = uuid4()
    cid_d = uuid4()
    empresa = EmpresaInfo(
        cnpj="33014556000196",
        razao_social="Mesma Razão Social LTDA",
        porte=PorteEmpresa.MEDIO,
        regime=RegimeTributario.LUCRO_PRESUMIDO,
        cnae_principal="4711302",
        uf="RJ",
    ...
    cmd = ComandoConsultarCnpj(
        tenant_id=tid,
        cnpj_14="33014556000196",
        idempotency_key="k-sem-diff",
        aplicar_no_diagnostico_id=cid_d,
    )
    out, aplicado = await uc.executar_e_materializar(cmd)
    assert aplicado is False
    assert out.consulta_id == mat.consulta_id
    cnpj_repo.atualizar_empresa_diagnostico_em_andamento.assert_not_called()
